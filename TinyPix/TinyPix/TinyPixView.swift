...
//  TinyPix
//
//  Created by Glorin Li on 2020/4/24.
...
class TinyPixView: UIView {
    ...
    private func touchedGridIndexFromTouches(_ touches: Set<UITouch>) -> GridIndex {
        ...
        if gridRect.contains(location) {
            ...
            result.column = Int(8 - (location.x * 8 / gridRect.size.width))
    ...
    private func toggleSelectedBlock() {
        if selectedBlockIndex.row != -1 && selectedBlockIndex.column != -1 {
            document.toggleState(row: selectedBlockIndex.row, colunm: selectedBlockIndex.column)
            setNeedsDisplay()
        }
    }
